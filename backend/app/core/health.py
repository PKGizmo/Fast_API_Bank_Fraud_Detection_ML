...
class HealthCheck:
    ...
    def __init__(self):
        ...
        # dictionary to store the functions that check the health of service
        # keys are going to be string and values are going to be callable.
        # These callable are going to be functions that take no arguments
        # Awaitable[bool] - it's going to be a function that takes no arguments
        # and returns an awaitable
        # Awaitable - coroutine that eventually returns a boolean
    ...
    async def check_service_health(
        ...
        async with self._lock:
            ...
            logger.error(
                f"Service {service_name} is unhealthy after {max_retries} attempts: {metrics['last_error']}"
            )
    ...
    async def check_all_services(self) -> Dict[str, Any]:
        ...
        for service, result in zip(services, results):
            if isinstance(result, Exception):
                healts_status["services"][service] = {
                    "status": ServiceStatus.UNHEALTHY,
                    "error": str(result),
                    "last_check": self._last_check[service].isoformat(),
                }
                healts_status["status"] = ServiceStatus.DEGRADED
            else:
                healts_status["services"][service] = {
                    "status": result,
                    "last_check": self._last_check[service].isoformat(),
                ...
                if result != ServiceStatus.HEALTHY:
                    healts_status["status"] = ServiceStatus.DEGRADED
        self._cached_status = healts_status
        self._last_check_time = current_time

        return healts_status
