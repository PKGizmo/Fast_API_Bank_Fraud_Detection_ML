...
class UserAuthService:
        ...
        try:
            ...
            for attempt in range(3):
                try:
                    ...
                    logger.error(
                        f"Failed to sent OTP email(attempt: {attempt + 1}): {e}"
                    )
                    if attempt == 2:
                        user.otp = ""
                        user.otp_expiry_time = None
    ...
    async def check_user_lockout(
        ...
        if current_time >= lockout_time:
            await self.reset_user_state(user, session, clear_otp=False)
            logger.info(f"Lockout period ended for the user {user.email}")
            return
    ...
    async def reset_password(
        ...
        try:
            ...
            logger.info(f"Password reset successfull for user {user.email}")
