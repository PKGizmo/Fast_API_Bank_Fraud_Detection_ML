        ...
        if user:
            ...
            if not await user_auth_service.verify_user_password(
                ...
                if remaining_attempts > 0:
                    error_message = f"Invalid credentials. You have {remaining_attempts} attempt{'s' if remaining_attempts != 1 else ''} remaining before your account is temporarily locked."
                else:
                    error_message = (
                        "Invalid credentials. Your account has been temporarily locked due to "
                        f"too many failed attempts. Please try again after {settings.LOCKOUT_DURATION_MINUTES} minutes."
                    )
                raise HTTPException(
