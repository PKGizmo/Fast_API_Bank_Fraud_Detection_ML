...
async def activate_user(
    ...
    try:
        ...
        if error_message == "Activation token expired":
            raise HTTPException(
                status_code=status.HTTP_410_GONE,
                detail={
                    "status": "error",
                    "meesage": "Activation link has expired",
        ...
        elif error_message == "Invalid activation token":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={
                    "status": "error",
                    "meesage": "Invalid activation token",
                    "action": "Please confirm that the link you clicked on is correct",
                },
            )
        elif error_message == "User already activated":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={
                    "status": "error",
                    "meesage": "User already activated",
        ...
        raise HTTPException(
            ...
            detail={
                "status": "error",
                "meesage": "Failed to activate user account",
